...
function createtime1() {
  var grt = new Date("01/29/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
if (m == 8 && dd == 11) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝小G" + (y - 2002).toString() + "岁生日快乐！");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 6 && dd == 30) {//小猫咪生日
...
function createtime() {
  ...
  //var start = new Date("08/01/2025 00:00:00"); // 旅行者1号开始计算的时间
  //var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  //var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("01/29/2025 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://img.gofan.dpdns.org/blog/pc_day_background.webp);
    --darkmode-bg: url(https://img.gofan.dpdns.org/blog/pc_night_background.webp);
    --mobileday-bg: url(https://img.gofan.dpdns.org/blog/YunxiaoDay.webp);
    --mobilenight-bg: url(https://img.gofan.dpdns.org/blog/YunxiaoNight.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
